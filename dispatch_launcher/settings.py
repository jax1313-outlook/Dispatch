...
def collect_settings(*, facts: RuntimeFacts | None = None) -> SettingsView:
    ...
    for name, purpose, fallback in (
        ...
        ("DISPATCH_MEMORY_ROOT", "The Library shelf: evidence, receipts and the Library's documents.", "the portal data folder"),
        ("DISPATCH_LIBRARY_CATALOG", "The Library catalog (SQLite) the Portal reads and writes through.", "not configured: the Portal keeps its legacy library.json"),
