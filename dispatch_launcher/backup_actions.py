...
def status_line() -> str:
    """One line for the launcher: the truth word, then why it says that.

    `BackupStatus` carries `state` and `detail` and no renderer of its own --
    the maintenance page and the status screen each format the pair themselves.
    This keeps to the same shape so the launcher, the page and the screen cannot
    disagree about what the backup situation is.
    """
    status = backups.backup_status(resolve_backup_dir())
    line = f"{status.state}: {status.detail}"
    return f"{line} ({status.location})" if status.location else line
