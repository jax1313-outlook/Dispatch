...
## 2026-09-15 — Cockpit voice capture; loads held without a rate; no rate question; D1 refined for shared links

**PR:** (this change)
**Capability:** `portal/voice_capture.py` (new), `POST /portal/voice-capture`, the TALK card and voice drawer in `portal/templates/joe_portal.html`, `dispatch/opportunity.py` (`REQUIRED`, `KEY_FACTS`, `classify`, `one_question`, `echo`), `dispatch/listing.py`, `dispatch/alert_reader.py`, `portal/models/load_alerts.py`, `dispatch/scoring.py` (`RATE_POINTS`, `MAX_SCORE_WITHOUT_RATE`), `dispatch/load_assessment.py`, `portal/models/opportunity_card.py`, `portal/helpers.py`, card and brief templates, `portal/routes/joe_api.py`, `adapters/joe_connector.yaml`, `adapters/dispatch_session.py`, `docs/operations/COCKPIT_VOICE_CAPTURE.md`, `docs/operations/LOAD_ALERT_EMAILS.md`.
**Approved by:** Mike (owner)
**Approval, verbatim:**
- Voice: *"Interesting - a second voice channel. can this be created wholly inside of Dispatch? separate from microsoft? used to capture load board information?"*, *"i use head set."*, *"Start the voice mic button on the cockpit next"*; then *"clear expired cards too"*.
- No rate: *"after working through the truck smarter test many of the loads are listed with no rate amount. so if i am interested in the load it will require both email and phone call to acquire this information. this presents 2 issues. holding a load with out a rate will need an astric or blank is not negative."* On "LANE NOT HEARD": *"i do not know where this came from. I do not use lane for any thing. delete."* Then on three questions: *"1a, 2a, 3 yes stop asking rate"* — 1a: no lane message and no question, save what was heard; 2a: score from everything except the rate, marked "* rate pending"; 3: Joe stops asking "RATE?".
- D1: *"alter/ refine rule to include share links and the ability to open and recover any and all data required. it is assumes that shared links are permission to use."* On the options that followed: *"use share link"*.

**Voice.** A TALK card on the Driver Cockpit opens a voice drawer: TAP TO TALK where the browser offers speech recognition, and always a text box for the keyboard's microphone. Only words reach Dispatch — no audio, no speech model. "next" separates loads. Each load goes through the dictation reader and the capture contract on the ratified VOICE channel, with the contract's audit entry, and is read back declaratively ("RATE PENDING" when there is no rate). Voice clears expired uncommitted cards like paste. **Capture only:** it never commits, passes or sends. Browsers allow TAP TO TALK only on a secure or local connection; over the truck network the keyboard microphone works until the tablet connection is secured.

**No rate, no lane refusal, no question.** A capture is refused only when it carries no freight fact at all. The published contract's required list is empty. A missing rate or city is unknown, never a duplicate match. Joe asks nothing. A card without a rate shows Rate "*", is scored on every other factor against a 60-point maximum ("52 of 60 · * rate pending"), carries the neutral line "* Rate pending" rather than a warning, and never gets the below-floor warning. The inquiry threshold is compared as a share of the applicable maximum. Paste and alerts card loads with no rate or city.

**Doctrine superseded (text not edited here).** `docs/campaign/OPPORTUNITY_CAPTURE_PLAN.md` §2 ("only board, origin, destination, and rate are required") and §6 ("Joe asks at most one question, and only for the rate") are superseded by the rulings above.

**D1 refined.** Board sweeping still needs an API or MCP connection; no scraping. A load link Mike shares is permission to use it, and Dispatch may open it and recover the data it needs. **Finding, same day:** a TruckSmarter "public" share link redirects to TruckSmarter's sign-in, so reading one needs a signed-in TruckSmarter session. Mike chose the share-link path; the reader is not built yet and will be set up with him (his sign-in, his account).
...
## 2026-09-15 — One card per delivery; stops are no longer entered

**PR:** (this change)
**Capability:** `dispatch/mission_template.py` (31 fields; `consignee`, `bol_number`; `SHIPPER_KEYS`; stop blocks removed), `dispatch/shipper_group.py` (new), `portal/brief.py`, `portal/cockpit.py`, `portal/routes/joe_portal.py` (`GET /intake/another/<record_id>`), `portal/templates/mission_intake.html`, `portal/templates/mission_brief.html`, `portal/templates/joe_portal.html`.
**Approved by:** Mike (owner)
**Approval, verbatim:** *"a customer has 3 pallets for it's own customers at three different car dealerships ... Top right corner is 1 of 3, 2 of 3, 3 of 3. the customer is the same but each delivery is a different location and a different Bill of Lading. because the consignee is different. all three signed Bill of Lading is returned to the shipper as POD."*; *"rain stops one stop from completing so the driver returns with one load still onboard. This is why each must stand alone totally. the only binding item is the shipper. Everything thing else stands alone."*; *"Nothing about it binds them together. this is why i created the card system in the first place and used deterministic algorythm to process. they are event activated. button, click, voice, email. One card pre load mission."*; *"2 out of three get paid that day."*; *"The '1 of 3' label on each card that is enough! not more!"*; *"a new appointment for one that rolls to the next day, is still the same pallet, still going to the same location and it is still the <OPEN LOAD.> using the same everything except a different day. Which may be a week later."*; on load numbers, *"YES!!! EITHER FROM THE SHIPPER OR WE CREATE IT. the reason for the load number is that when i close a load and the POD and all invoice and closing documents are sent. a duplicate along with all original are placed in a folder inside of of Library according to that number for retervial from Archive. that is the tracing number. same system used by FedEx/ UPS and others."*; and *"do not over computer it"*.

**The model.** One card per delivery. Each card is a whole mission: its own consignee, address, appointment, freight, BOL, signed POD, invoice and payment, and its own load number (the shipper's if given, else generated). The only shared things are the shipper and the pickup. A rolled delivery is the same card with a new appointment; it stays OPEN, and nothing is copied.

**Built.** Two fields lead the DELIVERY section — Consignee and BOL number (31 fields). "ANOTHER DELIVERY FOR THIS SHIPPER" on the New Mission screen and on a saved brief opens a fresh card carrying only the shipper and pickup facts, with consignee, BOL, delivery, freight, rate and notes blank. A plain tag (`shipper_group`, the first card's load number) shows "1 of 3" on the card, the brief and the cockpit banner and binds nothing else: committing, delivering, discarding, invoicing or re-appointing one card leaves its siblings untouched, proven by test.

**Stops removed as an entry concept.** The stop blocks are gone from the New Mission screen, the brief and the emailed intake template, and the cockpit's stop selector with them. **Stored stop data is not deleted or rewritten**: an older multi-stop record still loads and renders.

**Recorded, not resolved.** Documents that still describe multi-stop missions: `MISSION_INTAKE_ARCHITECTURE.md`, `DRIVER_COCKPIT_LOCKED_DIRECTION.md` (a *locked direction* drawing the stop selector — its removal may want an explicit ruling), `docs/DISPATCH_LOAD_ARRANGEMENT_SPEC.md`, `docs/DISPATCH_POLICY_FOUNDATION_PR_SUMMARY.md`, `TRUCK_ARRANGEMENT_AND_LOAD_CONFIGURATION_ARCHITECTURE.md`, and the 2026-09-15 entry above on stop numbering. **Open for Mike:** rate starts blank on a second delivery (payment type, payor and amount carry over); the Loads list rows do not show the group label; an older record still shows its stored per-stop tag beside DELIVERY DETAILS.
